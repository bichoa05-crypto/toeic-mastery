    ...
    <div className="flex flex-col gap-5 rounded-2xl border border-border bg-card p-5 shadow-soft sm:p-6">
      ...
      {hasSharedPassage && passage && (
        <div key={question.passageId}>
          {passage.audioUrl ? (
            <AudioPlayer src={passage.audioUrl} allowReplay={mode === "PRACTICE" || allowReplay} className="mb-3" />
          ) : (
            passage.transcript && (
              <TtsAudioPlayer text={passage.transcript} allowReplay={mode === "PRACTICE" || allowReplay} className="mb-3" />
            )
          )}
      ...
      {!passage?.audioUrl && !passage?.transcript && (
        question.audioUrl ? (
          <AudioPlayer src={question.audioUrl} allowReplay={mode === "PRACTICE" || allowReplay} />
        ) : (
          question.transcript && <TtsAudioPlayer text={question.transcript} allowReplay={mode === "PRACTICE" || allowReplay} />
        )
      )}
